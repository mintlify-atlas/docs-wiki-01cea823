...
| `fineract-security/…/domain/OTPRequestRepository.java` | `org.apache.fineract.infrastructure.security.domain` | In-memory `@Repository` backed by a `ConcurrentHashMap<Long, OTPRequest>` keyed by `AppUser.getId()`; **not** a JPA repository — OTP requests are never persisted to the database |
...
All runtime values are stored in the **`twofactor_configuration`** table (a dedicated 2FA key-value table, not `m_configuration`) and are read via `TwoFactorConfigurationService`. The JPA entity is `TwoFactorConfiguration` (`@Table(name = "twofactor_configuration")`). Constants are defined in `TwoFactorConfigurationConstants`:
...
**File:** `fineract-security/src/main/java/org/apache/fineract/infrastructure/security/domain/OTPRequestRepository.java`

Unlike `TFAccessTokenRepository`, this is **not** a Spring Data JPA repository. It is a plain `@Repository`-annotated Spring bean backed by an in-memory `ConcurrentHashMap`:
...
OTP requests are keyed by `AppUser.getId()` and held **only in application memory** — they are not persisted to any database table. This means pending OTPs are lost on application restart. In a multi-node deployment, all requests for a given user must route to the same node, or a distributed cache must be substituted.
...
<Accordion title="OTP requests are in-memory only — not persisted">
`OTPRequestRepository` is backed by a `ConcurrentHashMap`, not a database table. In-flight OTP codes are lost on application restart or JVM crash. In clustered deployments, sticky sessions are required so that the `POST /v1/twofactor/validate?token=...` call reaches the same node that stored the OTP. This is different from `TFAccessToken`, which is persisted to `twofactor_access_token` and is safe for multi-node deployments.
</Accordion>
