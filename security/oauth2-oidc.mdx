...
Extends `OncePerRequestFilter`. Registered before `SecurityContextHolderFilter` in `OidcFederationSecurityConfig`. The filter parses the Bearer JWT (without signature verification) to find a tenant and then calls `ThreadLocalContextUtil.setTenant()`. Always calls `ThreadLocalContextUtil.reset()` in a `finally` block.

Full tenant resolution priority (from the `resolveTenantId` method):

1. **JWT `iss` → master DB (`m_tenant_oidc_config`)**: extracts the `iss` claim from the Bearer JWT and calls `TenantOidcConfigService.findByIssuerUri(iss)` — returns `config.getTenantId()` if a matching record is found.
2. **JWT `iss` → YAML `issuers[]`**: if not found in DB, scans `fineractProperties.getSecurity().getOidcFederation().getIssuers()` for a matching `issuerUri` and returns its `tenantId`.
3. **Configurable JWT claim** (`fineract.security.oidc-federation.tenant-claim-name`, default `fineract_tenant`): reads the named claim directly from the JWT payload.
4. **`Fineract-Platform-TenantId` HTTP header**.
5. **`tenantIdentifier` query parameter**.

The filter is skipped for `/swagger-ui`, `/actuator`, `/login`, and `/error` paths. If resolution fails for any reason, the filter logs a warning and calls `chain.doFilter()` — downstream authentication will fail with an appropriate error if tenant context is required.
...
sequenceDiagram
    ...
    OidcTenantAwareFilter->>TenantOidcConfigService: findByIssuerUri(iss) — Priority 1
    alt iss matches DB record
        TenantOidcConfigService-->>OidcTenantAwareFilter: tenantId from m_tenant_oidc_config
    else iss matches YAML issuers[]
        OidcTenantAwareFilter->>OidcTenantAwareFilter: tenantId from YAML config — Priority 2
    else fineract_tenant claim present
        OidcTenantAwareFilter->>OidcTenantAwareFilter: tenantId from JWT claim — Priority 3
    else header / query param
        OidcTenantAwareFilter->>OidcTenantAwareFilter: tenantId from header/param — Priority 4/5
    end
...
`OidcTenantAwareFilter` runs before Spring Security validates the JWT signature. Its primary resolution strategy is to match the JWT `iss` claim against `m_tenant_oidc_config` (DB first, then YAML `issuers[]`). Only if `iss`-based lookup fails does it fall back to the configurable `fineract_tenant` claim, then `Fineract-Platform-TenantId` header, then `tenantIdentifier` query parameter. If all five strategies fail, tenant context is not set, and subsequent domain service calls will fail because `ThreadLocalContextUtil` has no tenant — even if JWT signature validation succeeds.
</Accordion>
