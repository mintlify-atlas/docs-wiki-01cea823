...
<Steps>
  ...
  <Step title="Apply lock">
    Worker's `ApplyLoanLockTasklet` acquires a row in `loan_account_locks` for every loan ID in the partition with `lock_owner = LOAN_COB_CHUNK_PROCESSING`.
  </Step>
...
Extends `AbstractLoanItemWriter`. Saves the processed `Loan` via `LoanRepository` (Spring Data `save()`). After write, changes the lock owner from `LOAN_COB_CHUNK_PROCESSING` to allow the account to be accessed again (the final unlock happens in `UnlockProcessedLoansTasklet`).

## Loan COB Locking (`loan_account_locks` table)

The `AccountLock` mapped superclass drives the lock mechanism:

```java
// fineract-cob/…/cob/domain/AccountLock.java
@MappedSuperclass
public abstract class AccountLock implements Persistable<Long> {
...
`COBApiFilter` / `LoanCOBApiFilter` intercepts incoming API requests targeting a locked loan and returns HTTP 409 with a `LoanIsLockedByAntiStarvationException` until the lock is released.
...
sequenceDiagram
    ...
    loop for each partition
        ...
        W->>DB: ApplyLoanLockTasklet (INSERT loan_account_locks)
...
`LoanCOBApiFilter` is a JAX-RS `ContainerRequestFilter` registered in `fineract-provider`. It inspects incoming requests for loan-affecting operations, checks `loan_account_locks` via `LoanCOBFilterHelperImpl`, and if the target loan has `lock_owner = LOAN_COB_CHUNK_PROCESSING`, it triggers inline COB processing for that loan before allowing the request to proceed. This is the anti-starvation mechanism: a loan that was locked mid-batch can still be operated on from the API immediately.
