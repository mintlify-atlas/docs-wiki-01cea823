...
|-------|---------|-------------|
| `0` | `INVALID` | Sentinel |
| `100` | `INITIATED` | Dividend created, not posted |
| `200` | `POSTED` | Dividend posted to savings |
| `300` | `REJECTED` | Dividend rejected |
...
|-------|---------|-------------|
| `0` | `INVALID` | Sentinel |
| `100` | `INITIATED` | Payout record created |
| `200` | `POSTED` | All account dividends posted |
| `300` | `REJECTED` | Dividend rejected |
...
sequenceDiagram
    ...
    participant ProductSvc as ShareProductCommandsServiceImpl
...
Table: `m_share_product_dividend_pay_out_details`

Key fields: `shareProductId`, `dividendPeriodStartDate`, `dividendPeriodEndDate`, `dividendAmount`, `status` (`ShareProductDividendStatusType`).
...
Key fields: `shareAccountId`, `savingsAccountId`, `dividendPayOutDetailsId`, `amount`, `status` (`ShareAccountDividendStatusType`).
...
<Note>
Dividend approval (`PUT /shareproduct/{productId}/dividend/{dividendId}?command=approve`) is separate from actual posting. Approval creates the `m_share_account_dividend_details` rows with `status=INITIATED`. The nightly job `PostDividentsForSharesTasklet` then picks these up and posts them. If the job fails mid-run, partially-posted dividends leave some rows at `INITIATED` — re-running the job is safe as it only processes `INITIATED` rows.
</Note>
