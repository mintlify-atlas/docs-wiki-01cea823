...
graph TB
    ...
    subgraph CQRS["CQRS Command Bus (fineract-core + fineract-command)"]
        ...
        CHP["CommandHandlerProvider\nlookup by @CommandType(action,entity)"]
...
sequenceDiagram
    ...
    AR->>CWB: new CommandWrapperBuilder().<br/>disburseLoan(loanId).build()
    ...
    SCPS->>CHP: getHandler(actionName, entityName)
...
| `CommandWrapperBuilder` | `fineract-core/.../commands/service/CommandWrapperBuilder.java` | Fluent builder — 300+ methods, one per business operation (e.g., `disburseLoan()`, `approveApplication()`) |
...
| `CommandHandlerProvider` | `fineract-core/.../commands/provider/CommandHandlerProvider.java` | Registry of `NewCommandSourceHandler` beans keyed by `@CommandType(action, entity)` |
...
<Note>
The `LOAN_COB` job has a COB API filter: `LoanCOBApiFilter` in `fineract-provider/src/main/java/org/apache/fineract/infrastructure/jobs/filter/LoanCOBApiFilter.java`. Any API request that touches a loan currently being processed by COB will receive a `409 Conflict` response to prevent data races. The filter is registered as a Spring `OncePerRequestFilter`.
</Note>
...
This sequence diagram traces a `POST /v1/loans/{loanId}/transactions?command=disburse` request from HTTP reception through to database commit.
...
sequenceDiagram
    ...
    participant LTA as LoanTransactionsApiResource<br/>(fineract-provider)
    participant PCSWPS as PortfolioCommandSourceWritePlatformService<br/>(fineract-core)
    participant SCPS as SynchronousCommandProcessingService<br/>(fineract-core)
    participant CHP as CommandHandlerProvider<br/>(fineract-core)
    participant LATH as LoanDisbursementCommandHandler<br/>(fineract-loan)
    participant RDS as RoutingDataSource<br/>(fineract-core)
    participant DB as fineract_default PostgreSQL

    CLIENT->>TSEC: POST /fineract-provider/api/v1/loans/42/transactions<br/>?command=disburse<br/>Fineract-Platform-TenantId: default<br/>Authorization: Basic ...
    ...
    JC->>LTA: route to @Path("/v1/loans/{loanId}/transactions")<br/>@POST + @QueryParam("command")="disburse"
    LTA->>LTA: CommandWrapperBuilder.disburseLoan(loanId).build()
    LTA->>PCSWPS: logCommandSource(CommandWrapper)
    PCSWPS->>PCSWPS: authenticatedUser.validateHasPermissionTo("DISBURSE_LOAN")
    ...
    SCPS->>CHP: getHandler("DISBURSE", "LOAN")
    CHP-->>SCPS: LoanDisbursementCommandHandler bean
    ...
    SCPS-->>LTA: CommandProcessingResult
    LTA-->>CLIENT: 200 OK {"loanId": 42, "resourceId": 42}
```
