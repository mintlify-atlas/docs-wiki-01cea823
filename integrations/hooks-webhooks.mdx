...
Resolves processor bean by `hook.getTemplate().getName()`:

```java
switch (templateName) {
    case "SMS Bridge"       → TwilioHookProcessor
    case "Web"              → WebHookProcessor
    case "Elastic Search"   → ElasticSearchHookProcessor
    case "Message Gateway"  → MessageGatewayHookProcessor
    default                 → null  // NPE will occur — must be a known template
}
```
