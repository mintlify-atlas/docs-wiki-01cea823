...
| Enum Value | Meaning |
|-----------|---------|
| `THITSAWORKS` | Identifies the ThitsaWorks bureau implementation |
| `SUBSCRIPTIONID` | Bureau subscription/client ID |
| `SUBSCRIPTIONKEY` | Bureau subscription key |
...
<Steps>
  <Step title="Read configuration">
    Loads `CreditBureauConfiguration` rows for the bureau (from `m_creditbureau_configuration`). Configuration keys are defined in `CreditBureauConfigurations` enum: `USERNAME`, `PASSWORD`, `SUBSCRIPTIONID`, `SUBSCRIPTIONKEY`, `TOKENURL`, `CREDITREPORTURL`, etc.
  </Step>
  ...
  <Step title="Fetch or upload report">
    For report fetch: `POST` to `CREDITREPORTURL` with `SUBSCRIPTIONID`/`SUBSCRIPTIONKEY` + national ID. For upload: multipart `POST` to the bureau's report upload endpoint.
  </Step>
