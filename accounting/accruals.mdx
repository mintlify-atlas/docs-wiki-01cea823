...
Loan products store this value in the `accounting_type` column of `m_product_loan` (mapped to the `LoanProduct.accountingRule` field). It is surfaced at runtime via `LoanDTO.isPeriodicAccrualBasedAccountingEnabled()` and `LoanDTO.isUpfrontAccrualBasedAccountingEnabled()`.
...
Savings products can also use accrual accounting. When the savings accounting type is `ACCRUAL_PERIODIC`, the processor handles `SavingsTransactionDTO` events with `isAccrual()` type. The posting direction depends on whether the savings account is in overdraft:

- **Regular savings accrual:** DEBIT `INTEREST_RECEIVABLE`, CREDIT `INCOME_FROM_INTEREST`
- **Overdraft accrual:** DEBIT `INTEREST_ON_SAVINGS`, CREDIT `INTEREST_PAYABLE`
