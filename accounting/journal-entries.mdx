...
sequenceDiagram
    ...
    Proc->>Helper: helper.createDebitJournalEntryForLoan(...)
...
|---|---|
| `isReversed()` | `journalEntryWritePlatformService.createJournalEntryForReversedLoanTransaction()` — checked **first**; skips remaining checks via `continue` |
...
| `isWriteOff()` | Inline: DEBIT `LOSSES_WRITTEN_OFF`, CREDIT `LOAN_PORTFOLIO` (principal only) |
| `isChargeAdjustment()` | `createJournalEntriesForChargeAdjustment()` |
| `isChargeoff()` | `createJournalEntriesForChargeOff()` |
...
| `createDebitJournalEntryForLoan(...)` | Creates a DEBIT `JournalEntry` for a loan transaction |
| `createCreditJournalEntryForLoan(...)` | Creates a CREDIT `JournalEntry` for a loan transaction |
| `createJournalEntriesForLoan(...)` | Creates a balanced debit+credit pair in a single call |
| `getOfficeById(long officeId)` | Office lookup by ID (used in the per-transaction loop) |
...
| `getLinkedGLAccountForLoanProduct(Long loanProductId, int accountMappingTypeId, Long paymentTypeId)` | Looks up GL account from `ProductToGLAccountMappingRepository` by product, mapping type, and optional payment type |
