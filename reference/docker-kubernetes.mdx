...
| File | Purpose |
|---|---|
| `docker-compose.yml` | Default stack: PostgreSQL + Fineract (ports `8443`, `5000`) |
| `docker-compose-postgresql.yml` | PostgreSQL variant with explicit version pin |
...
```mermaid
graph LR
    kafka["kafka<br/>(apache/kafka:4.2.0)"]
    db["db<br/>(PostgreSQL)"]
...
| `fineractmysql-deployment.yml` | `PersistentVolume` + `PersistentVolumeClaim` + `Service` + `Deployment` | MariaDB 12.2 with 10Gi PV, headless ClusterIP service |
...
| `FINERACT_HIKARI_DRIVER_CLASS_NAME` | literal | `org.mariadb.jdbc.Driver` |
