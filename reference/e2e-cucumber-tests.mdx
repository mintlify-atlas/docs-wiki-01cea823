---
title: "Fineract E2E Cucumber Tests: Architecture and Usage"
sidebarTitle: "E2E Cucumber Tests"
description: "Architecture of the e2e-tests-core and e2e-tests-runner modules: feature files, step definitions, initializer lifecycle, and tag-based scenario filtering."
---

Apache Fineract's E2E test suite is built on Cucumber 7 with Spring Boot test context. It consists of two Gradle modules: `fineract-e2e-tests-core` (shared step definitions and infrastructure) and `fineract-e2e-tests-runner` (feature files, test runner, and environment-specific initializers). Together they provide 108 feature files covering the complete loan lifecycle, savings, COB, asset externalization, EMI calculations, and more.
...
<CardGroup cols={2}>
  <Card title="fineract-e2e-tests-core" icon="puzzle-piece">
    **Shared infrastructure** consumed by the runner and custom module tests:
    - 47 step definition classes (`*StepDef.java`) plus 5 hook classes
...
public class FineractClientConfiguration {
    ...
    public FineractFeignClient fineractFeignClient() {
        ...
        boolean debugEnabled = Boolean.parseBoolean(
            System.getProperty("fineract.feign.debug", "false"));
...
`ApiProperties` (`fineract-e2e-tests-core/src/test/java/org/apache/fineract/test/api/ApiProperties.java`) reads `fineract-test.api.*` properties. The `FineractFeignClient` class is from `fineract-client-feign`.
...
Step definitions live in `fineract-e2e-tests-core/src/test/java/org/apache/fineract/test/stepdef/`. There are 47 `*StepDef.java` classes plus 5 hook classes in the `hook/` sub-package.

### Loan Step Definitions (`stepdef/loan/`)

| Class | Covered Steps |
|---|---|
| `LoanStepDef.java` | Loan create/approve/disburse/repay/close, status assertions, schedule validation |
| `LoanRepaymentStepDef.java` | Repayment transactions, advance payments |
| `ChargeStepDef.java` | Charge product creation and configuration |
| `LoanChargeStepDef.java` | Loan-level charge creation, waiver, reversal |
| `LoanChargeBackStepDef.java` | Chargeback operations |
| `LoanChargeAdjustmentStepDef.java` | Charge adjustment |
| `LoanDelinquencyStepDef.java` | Delinquency bucket assertions |
| `LoanCOBStepDef.java` | COB job trigger, business date advance, inline COB |
| `InlineCOBStepDef.java` | Inline COB execution via API |
| `LoanOriginationStepDef.java` | Loan origination workflow |
| `LoanReAgingStepDef.java` | Re-aging operations |
| `LoanReAmortizationStepDef.java` | Re-amortization operations |
| `LoanRescheduleStepDef.java` | Loan reschedule request |
| `LoanReprocessStepDef.java` | Transaction reprocessing |
| `LoanInterestPauseStepDef.java` | Interest pause periods |
| `LoanCapitalizedIncomeStepDef.java` | Capitalized income handling |
| `LoanOverrideFieldsStepDef.java` | Override fields on loan |
...
| `WorkingCapitalDelinquencyConfigStepDef.java` | Working capital delinquency configuration |
| `WorkingCapitalDelinquencyRescheduleStepDef.java` | Working capital delinquency reschedule |
| `WorkingCapitalChargeStepDef.java` | Working capital charges |
| `WorkingCapitalBatchApiStepDef.java` | Working capital batch API |
| `WorkingCapitalAmortizationScheduleStepDef.java` | Working capital amortization schedule |
| `WorkingCapitalBreachScheduleStepDef.java` | Working capital breach schedule |
| `WorkingCapitalNearBreachConfigStepDef.java` | Near-breach configuration |
| `WorkingCapitalLoanActionTemplateStepDef.java` | Working capital loan action templates |

### Common Step Definitions (`stepdef/common/`)
...
| `WorkingCapitalLoanCobStepDef.java` | Working capital COB operations |

### Other Step Definitions

| Class | Domain |
|---|---|
| `SavingsAccountStepDef.java` (`stepdef/saving/`) | Savings account full lifecycle |
| `AssetExternalizationStepDef.java` (`stepdef/assetexternalization/`) | Loan asset buy/sell transfers |
| `AccountTransferStepDef.java` (`stepdef/account/`) | Account-to-account transfers |
| `DatatablesStepDef.java` (`stepdef/datatable/`) | Datatable CRUD |
| `ReportingStepDef.java` (`stepdef/reporting/`) | Report generation and validation |

### Hook Classes (`stepdef/hook/`)

| Class | Purpose |
|---|---|
| `InitializingHook.java` | Spring context initialization and suite setup |
| `TestContextLifecycleHook.java` | Clears/restores `TestContext` before/after each scenario |
| `MessagingHook.java` | JMS connection lifecycle |
| `TestRailLifecycleHook.java` | TestRail result reporting |
| `ConnectionPoolDiagnosticsHook.java` | Connection pool diagnostics logging |

---

## Feature File Inventory

All 108 feature files live in `fineract-e2e-tests-runner/src/test/resources/features/`:
...
| `LoanReAgingEqualAmortization-Part1.feature` through `Part3.feature` | Re-aging with equal amortization |
| `LoanReAgingPreview.feature` | Re-aging preview |
| `LoanReAgingAccruals.feature` | Accruals after re-aging |
| `LoanReAgingAccrualsEqualAmortization.feature` | Re-aging accruals with equal amortization |
| `LoanReAmortization-Part1.feature`, `LoanReAmortization-Part2.feature` | Re-amortization |
| `LoanReAmortizationPreview.feature` | Re-amortization preview |
...
Over 30 feature files covering the complete working capital loan system:

| Feature File | Content Summary |
|---|---|
| `WorkingCapitalLoanAccount.feature` | Working capital account lifecycle |
| `WorkingCapitalLoanProduct.feature` | Working capital loan product CRUD |
| `WorkingCapitalLoanProductAdvancedAccounting.feature` | Advanced accounting for WC products |
| `WorkingCapitalLoanRepayment.feature` | Repayment flows |
| `WorkingCapitalLoanRepaymentAccountingEntries.feature` | Accounting entries during repayment |
| `WorkingCapitalDelinquency.feature` | Delinquency tracking |
| `WorkingCapitalDelinquencyConfiguration.feature` | Delinquency configuration |
| `WorkingCapitalDelinquencyPause.feature` | Delinquency pause |
| `WorkingCapitalDelinquencyReschedule.feature` | Delinquency reschedule |
| `WorkingCapitalBreachConfiguration.feature` | Breach configuration |
| `WorkingCapitalBreachEvaluation.feature` | Breach evaluation |
| `WorkingCapitalBreachSchedule.feature` | Breach schedule |
| `WorkingCapitalNearBreachConfiguration.feature` | Near-breach configuration |
| `WorkingCapitalNearBreachEvaluation.feature` | Near-breach evaluation |
| `WorkingCapitalDiscount.feature` | Discount operations |
| `WorkingCapitalDiscountAdjustment.feature` | Discount adjustment |
| `WorkingCapitalDiscountAdjustmentBackdateUndo.feature` | Backdate/undo discount adjustment |
| `WorkingCapitalDiscountAmortization.feature` | Discount amortization |
| `WorkingCapitalDiscountAmortizationAdjustment.feature` | Discount amortization adjustment |
| `WorkingCapitalAmortizationSchedule.feature` | Amortization schedule |
| `WorkingCapitalPeriodPaymentRate.feature` | Period payment rate |
| `WorkingCapitalBatchApi.feature` | Batch API for working capital |
| `WorkingCapitalDatatables.feature` | Datatables for working capital |
| `WorkingCapitalLoanCharge.feature` | Working capital loan charges |
| `WorkingCapitalLoanCreditBalanceRefund.feature` | Credit balance refund |
| `WorkingCapitalLoanGoodwillCredit.feature` | Goodwill credit |
| `WorkingCapitalLoanGoodwillCreditAccountingEntries.feature` | Goodwill credit accounting |
| `WorkingCapitalLoanActionTemplates.feature` | Loan action templates |
