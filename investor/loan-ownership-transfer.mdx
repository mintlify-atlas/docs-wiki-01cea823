...
description: "Deep dive into Apache Fineract's investor module: entities, API endpoints, COB business step, accounting service, Avro events, and transfer lifecycle state machine."
---
...
`ExternalTransferDataDetails` holds the seven outstanding-balance `BigDecimal` fields from `ExternalAssetOwnerTransferDetails`.
...
**Table:** `m_external_asset_owner_loan_product_attributes` (via `ExternalAssetOwnerLoanProductAttributes` entity)
...
The outstanding interest strategy is read from the global configuration key `"asset-externalization-of-non-active-loans"` (via `ConfigurationDomainService.getAssetOwnerTransferOustandingInterestStrategy()`). The two strategies are implemented in `ExternalAssetOwnerTransferOutstandingInterestCalculation`.
...
| Global config: outstanding interest strategy | — | `TOTAL_OUTSTANDING_INTEREST` or `PAYABLE_OUTSTANDING_INTEREST` |
...
The `ExternalAssetOwnerLoanProductAttributesApiResource` at `/v1/external-asset-owners/loan-product` manages per-product attributes stored in `m_external_asset_owner_loan_product_attributes`. The `SETTLEMENT_MODEL` attribute gates the intermediary/delayed-settlement code path. Both `DelayedSettlementAttributeServiceImpl` and `ExternalAssetOwnerTransferOutstandingInterestCalculation` read from this table.
</Accordion>
