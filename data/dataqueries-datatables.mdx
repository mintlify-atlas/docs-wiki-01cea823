...
description: "Fineract's datatable subsystem: EntityTables enum, EntityDatatableChecks enforcement, GenericResultsetData, all CRUD endpoints, and the AdHocApiResource."
---
...
| `AdHocApiResource.java` | `...adhocquery.api` | REST resource – `/v1/adhocquery` CRUD |
...
| `StatusEnum.java` | `...dataqueries.data` (core) | Lifecycle status integers used in entity-datatable checks |
| `AdHoc.java` | `...adhocquery.domain` | JPA entity – `m_adhoc` (scheduled SQL queries) |
...
public class EntityDatatableChecks extends AbstractPersistableCustom<Long> {
    ...
    Integer status;          // status_enum (lifecycle status integer from StatusEnum)
    boolean systemDefined;
    Long    productId;       // nullable – restrict to specific product
}
```

When a lifecycle transition fires (e.g. loan approval at `status = 200`), `EntityDatatableChecksWritePlatformServiceImpl.runTheCheck()` is invoked. It queries `m_entity_datatable_check` for rows matching `entity` + `status_enum`, then for each match queries the datatable for rows with the entity FK. If no rows exist, it throws `DatatableEntryRequiredException`.
...
public final class GenericResultsetData {
    ...
    List<ResultsetRowData>          data;          // rows of string values
}
```

`ResultsetColumnHeaderData` carries: `columnName`, `columnType` (`JdbcJavaType`), `columnLength`, `isColumnNullable`, `isColumnPrimaryKey`, `isColumnUnique`, `isColumnIndexed`, `columnValues` (for Dropdown/code columns), `columnCode`. It also exposes a `columnDisplayType` field (derived from `JdbcJavaType`) used to drive the UI rendering hint.

### `DatatableReadService` / `DatatableWriteService` Interfaces

Both interfaces live in `fineract-core/.../dataqueries/service/`. Key method signatures:
...
Page<JsonObject> queryDataTableAdvanced(String datatable, PagedLocalRequest<AdvancedQueryData> pagedRequest);
...
`fineract-core/.../dataqueries/data/StatusEnum.java` defines all valid integer values for `m_entity_datatable_check.status_enum`:

| Enum Constant | Integer Value | Meaning |
|---------------|---------------|---------|
| `CREATE` | 100 | Entity created (pending) |
| `APPROVE` | 200 | Entity approved |
| `ACTIVATE` | 300 | Entity activated |
| `WITHDRAWN` | 400 | Application withdrawn |
| `REJECTED` | 500 | Application rejected |
| `CLOSE` | 600 | Entity closed |
| `WRITE_OFF` | 601 | Loan written off |
| `RESCHEDULE` | 602 | Loan rescheduled |
| `OVERPAY` | 700 | Loan overpaid |
| `DISBURSE` | 800 | Loan disbursed |

### `EntityTables` Enum

`fineract-core/.../dataqueries/data/EntityTables.java` enumerates all valid `apptableName` values and their foreign-key column names. The `Supported Check Statuses` column shows which `StatusEnum` integers can be used in `m_entity_datatable_check.status_enum` for that entity; blank = checks not supported.

| Enum Value | DB Table (`name`) | FK Column on Datatable | Supported Check Statuses |
|------------|-------------------|------------------------|--------------------------|
| `CLIENT` | `m_client` | `client_id` | CREATE(100), ACTIVATE(300), CLOSE(600) |
| `GROUP` | `m_group` | `group_id` | CREATE(100), ACTIVATE(300), CLOSE(600) |
| `CENTER` | `m_center` (apptable: `m_group`) | `center_id` | *(none)* |
| `OFFICE` | `m_office` | `office_id` | *(none)* |
| `LOAN_PRODUCT` | `m_product_loan` | `product_loan_id` | *(none)* |
| `LOAN` | `m_loan` | `loan_id` | CREATE(100), APPROVE(200), DISBURSE(800), WITHDRAWN(400), REJECTED(500), WRITE_OFF(601) |
| `SAVINGS_PRODUCT` | `m_savings_product` | `savings_product_id` | *(none)* |
| `SAVINGS` | `m_savings_account` | `savings_account_id` | CREATE(100), APPROVE(200), ACTIVATE(300), WITHDRAWN(400), REJECTED(500), CLOSE(600) |
| `SAVINGS_TRANSACTION` | `m_savings_account_transaction` | `savings_transaction_id` | *(none)* |
| `SHARE_PRODUCT` | `m_share_product` | `share_product_id` | *(none)* |
| `WC_LOAN_PRODUCT` | `m_wc_loan_product` | `wc_product_loan_id` | *(none)* |
| `WC_LOAN` | `m_wc_loan` | `wc_loan_id` | *(none)* |

<Note>
`CENTER` uses a four-argument constructor: `CENTER("m_center", "m_group", "center_id", "id")`. Its `apptableName` is `m_group` (the parent table for API lookups) while the actual database table is `m_center`. The FK column on any linked datatable is `center_id`.
</Note>
...
The `AdHocApiResource` lives in `fineract-provider/.../adhocquery/api/` and manages saved ad-hoc SQL queries stored in `m_adhoc`. These are distinct from `stretchy_report` entries — they are intended for scheduled SQL execution with email delivery rather than on-demand reporting.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/v1/adhocquery` | List all ad-hoc queries. |
| `GET` | `/v1/adhocquery/template` | Retrieve creation template. |
| `GET` | `/v1/adhocquery/{adHocId}` | Retrieve one ad-hoc query by ID. |
| `POST` | `/v1/adhocquery` | Create a new ad-hoc query. Fields: `name`, `query`, `tableName`, `tableField`, `email`, `reportRunFrequency`, `isActive`. |
| `PUT` | `/v1/adhocquery/{adHocId}` | Update an existing ad-hoc query. |
| `DELETE` | `/v1/adhocquery/{adHocId}` | Delete an ad-hoc query. |

The `AdHoc` entity (`m_adhoc`) stores: `name` (max 100), `query` (SQL, max 2000), `tableName` (max 100), `tableFields` (max 2000), `email` (max 500), `reportRunFrequency` (FK → `ReportRunFrequency`), `isActive`.
...
| `Dropdown` | `INT` (FK → `m_code_value`) | Requires `code` field. Column name becomes `<code>_cd_<columnName>`. |
| `Number` | `BIGINT` | |
| `String` | `VARCHAR(length)` | Requires `length` field. |
| `Text` | `TEXT` | |

<Note>
Dropdown columns store the code-value ID (integer). The actual DB column name is computed as `<code>_cd_<columnName>` by `DatatableWriteServiceImpl.datatableColumnNameToCodeValueName()`. For example, a column named `gender` with `code = "Gender"` becomes `Gender_cd_gender`. The corresponding code value label is resolved by joining `m_code_value` at query time via `GenericDataServiceImpl.fillResultsetColumnHeaders()`.
</Note>
...
For product-scoped checks (e.g. a loan with a specific `productId`), `runTheCheckForProduct()` additionally filters by `product_id`.
...
| `report_subtype` | `VARCHAR(20)` | Required when `report_type=Chart`; must be `Bar` or `Pie`. Must be blank for non-Chart types. |
...
| `report_sql` | `TEXT` | Required for `Table` and `Chart` types; must be blank for `Pentaho` and `SMS` |
...
<Warning>
**Dropdown columns are renamed.** When you declare a column of type `Dropdown` with `name = "gender"` and `code = "Gender"`, the actual DB column becomes `Gender_cd_gender` — the formula is `<code>_cd_<columnName>`, applied by `DatatableWriteServiceImpl.datatableColumnNameToCodeValueName()`. API requests must use the suffixed name.
</Warning>
