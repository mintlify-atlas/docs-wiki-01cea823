...
| `BulkImportEvent.java` | `...bulkimport.data` | Spring event carrying `Workbook`, locale, dateFormat, `ImportDocument` |
...
`fineract-core/.../bulkimport/data/GlobalEntityType.java` defines all importable types:

| Enum | Code | Value |
|------|------|-------|
| `CLIENTS_PERSON` | `clients.person` | 1 |
| `CLIENTS_ENTITY` | `clients.entity` | 2 |
| `GROUPS` | `groups` | 3 |
| `CENTERS` | `centers` | 4 |
| `OFFICES` | `offices` | 5 |
| `STAFF` | `staff` | 6 |
| `USERS` | `users` | 7 |
| `LOANS` | `loans` | 15 |
| `LOAN_TRANSACTIONS` | `loantransactions` | 18 |
| `GUARANTORS` | `guarantors` | 19 |
| `SAVINGS_ACCOUNT` | `savingsaccount` | 23 |
| `SAVINGS_TRANSACTIONS` | `savingstransactions` | 25 |
| `GL_JOURNAL_ENTRIES` | `gljournalentries` | 27 |
| `CHART_OF_ACCOUNTS` | `chartofaccounts` | 30 |
| `FIXED_DEPOSIT_ACCOUNTS` | `fixeddepositaccounts` | 31 |
| `FIXED_DEPOSIT_TRANSACTIONS` | `fixeddeposittransactions` | 32 |
| `SHARE_ACCOUNTS` | `shareaccounts` | 33 |
| `RECURRING_DEPOSIT_ACCOUNTS` | `recurringdeposits` | 34 |
| `RECURRING_DEPOSIT_ACCOUNTS_TRANSACTIONS` | `recurringdepositstransactions` | 35 |
...
<Steps>
  ...
  <Step title="BulkImportEvent Published">
    `BulkImportEvent` is published via Spring's `ApplicationContext.publishEvent()`. It carries the `Workbook` object, locale, dateFormat, `ImportDocument`, and entity ID.
  </Step>
