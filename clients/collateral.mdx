...
<Steps>
  <Step title="Loan is closed (repaid, written-off, or foreclosed)">
    The loan write service triggers collateral release processing.
  </Step>
  <Step title="LoanCollateralAssembler processes pledges">
    `LoanCollateralAssembler` (`fineract-provider/.../collateralmanagement/service/LoanCollateralAssembler.java`) iterates all `LoanCollateralManagement` rows linked to the loan.
  </Step>
  <Step title="isReleased flag set to true">
    Each `LoanCollateralManagement.isReleased` is set to `true` and the row is updated.
  </Step>
  <Step title="Client inventory restored">
    `ClientCollateralManagement.updateQuantityAfterLoanClosed(quantity)` is called, which performs `this.quantity = this.quantity.add(quantity)` — returning the pledged units to the client's free inventory.
  </Step>
</Steps>

<Info>
After closure, the `LoanCollateralManagement` row is retained (with `is_released = true`) for audit purposes. The quantity is not physically deleted until `DELETE /v1/loan-collateral-management/{id}` is called explicitly.
</Info>
...
- `ClientCollateralManagement.quantity` is **increased** by the released amount (via `updateQuantityAfterLoanClosed`).
