...
| `fineract-provider/.../group/domain/GroupTypes.java` | fineract-provider | CENTER/GROUP enum |
...
Defined in `fineract-provider/.../group/domain/GroupTypes.java`.

| Constant | Long ID | Code | Value String |
|---|---|---|---|
| `INVALID` | 0 | `lendingStrategy.invalid` | `invalid` |
...
| `command` | `CommandWrapper` Builder Call | Effect |
|---|---|---|
| `activate` | `builder.activateGroup(groupId)` | PENDING → ACTIVE |
| `close` | `builder.closeGroup(groupId)` | ACTIVE → CLOSED |
| `associateClients` | `builder.associateClientsToGroup(groupId)` | Add clients via `m_group_client` |
| `disassociateClients` | `builder.disassociateClientsFromGroup(groupId)` | Remove clients from group |
| `generateCollectionSheet` | (read-only, no write command) | Returns `JLGCollectionSheetData` |
| `saveCollectionSheet` | `builder.saveGroupCollectionSheet(groupId)` | Bulk repayment/disbursal |
| `assignStaff` | `builder.assignGroupStaff(groupId)` | Assign loan officer |
| `unassignStaff` | `builder.unassignGroupStaff(groupId)` | Remove loan officer |
| `assignRole` | `builder.assignRole(groupId)` | Assign a group role |
| `unassignRole` | `builder.unassignRole(groupId, roleId)` | Remove a group role |
| `updateRole` | `builder.updateRole(groupId, roleId)` | Update role attributes |
| `transferClients` | `builder.transferClientsBetweenGroups(groupId)` | Transfer clients between groups |
...
| `command` | `CommandWrapper` Builder Call | Effect |
|---|---|---|
| `activate` | `builder.activateCenter(centerId)` | PENDING → ACTIVE |
| `close` | `builder.closeCenter(centerId)` | ACTIVE → CLOSED |
| `associateGroups` | `builder.associateGroupsToCenter(centerId)` | Link groups to center |
| `disassociateGroups` | `builder.disassociateGroupsFromCenter(centerId)` | Unlink groups from center |
| `generateCollectionSheet` | (read-only, no write command) | Returns `JLGCollectionSheetData` for the center |
| `saveCollectionSheet` | `builder.saveCenterCollectionSheet(centerId)` | Bulk repayment/disbursal |
