...
| `INTERST_REBATE_OWED` | interest rebate owed (note: enum name has a typo — `INTERST` — preserved from source) |
| `LOAN_CLOSED` | loan fully closed |
| `LOAN_EDIT_MULTI_DISBURSE_DATE` | multi-tranche disbursal date edited |
| `LOAN_REFUND` | refund posted against loan |
| `LOAN_FORECLOSURE` | `forecloseLoan` — early closure via foreclosure |
</Accordion>
